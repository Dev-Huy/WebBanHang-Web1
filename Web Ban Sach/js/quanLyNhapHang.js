import { phieuNhapList, productList, ready } from "./loadData.js";
let productListInput = [];

ready.then(() => {
    renderTable(phieuNhapList); //đảm bảo dữ liệu đã được load xong
})
...
async function renderTable(returnList) {
...
function renderDetail(object) {
    ...
    object.productlist.forEach((i, index) => {
        const name = productList.find(h => (h.id === i.id)).title;
        tablePN += `<tr>
                        <td>${index + 1}</td>
                        <td>${i.id}</td>
                        <td>${name}</td>
...
function insertInput(productListInput, idValue) {
    ...
    phieuNhapList.forEach(i => {
        i.productlist.forEach(j => {
            ...
            productFind.total += j.amount;
        })
    })
    localStorage.setItem("products", JSON.stringify(productList));
    localStorage.setItem("goodsReceipts", JSON.stringify(phieuNhapList));
    return true;
}


document.querySelector("#nhapSanPham").addEventListener("submit", (e) => {
    e.preventDefault();
    if (e.submitter && e.submitter.id === "done_inputProductBtn") {
        insertProduct(productListInput);
...
function continueInsert(idValue) {
    const currentObject = phieuNhapList.find(i => i.id === idValue);
    ...
    document.querySelector("#nhapPhieu #done_inputBtn").addEventListener("click", (e) => {
        ...
        currentObject.state = "Đã Hoàn Thành";
        renderTable(phieuNhapList);
