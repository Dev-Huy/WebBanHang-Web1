import { account, hashPassword, ready } from "./loadData.js";
...
// Chờ dữ liệu tải xong rồi mới gắn sự kiện
ready.then(() => {
  const viewHistoryBtn = document.getElementById("view-history-btn");

  if (viewHistoryBtn) {
    viewHistoryBtn.addEventListener("click", () => {
      // Lấy thông tin người dùng mới nhất mỗi khi click
      ...
      if (userOrders.length === 0) {
        alert("Bạn chưa có lịch sử mua hàng để thực hiện chức năng này.");
        return;
      }

      // Tạo một giỏ hàng mới từ tất cả các sản phẩm trong lịch sử mua hàng
      const historyCart = [];
      const addedProductIds = new Set(); // Dùng để tránh thêm sản phẩm trùng lặp

      userOrders.forEach(order => {
        order.chitiet.forEach(item => {
          // Chỉ thêm sản phẩm nếu chưa có trong giỏ hàng lịch sử
          if (!addedProductIds.has(item.id)) {
            // Đặt số lượng mặc định là 1 khi thêm vào giỏ hàng
            historyCart.push({ ...item, qty: 1 }); 
            addedProductIds.add(item.id);
          }
        });
      });

      // Lưu giỏ hàng này vào localStorage để trang thanh toán sử dụng
      localStorage.setItem("productSelected", JSON.stringify(historyCart));
      
      // Chuyển hướng đến trang thanh toán
      alert("Đã thêm tất cả sản phẩm bạn từng mua vào giỏ hàng. Chuyển đến trang thanh toán...");
